    ...
    try:
        ...
        if os.path.exists(md_file_path):
            ...
            if os.path.getsize(md_file_path) > 0:
                ...
                console_write(f"Found empty MD file: {md_file_path}, will reprocess", level="warning")
    ...
    try:
        ...
        for md_file in output_root.rglob("*.md"):
            try:
                ...
                console_write(f"Error processing MD file {md_file}: {e}", level="error")
                continue
    except Exception as e:
        ...
        async with self.md_gen_semaphore:
            try:
                if status == 'success':
                    ...
                    if original_cells:
                        original_cells = self._merge_adjacent_text_blocks_in_same_page(original_cells)
                        result['cells'] = original_cells
                    ...
                    if cells_to_render:
                        cells_to_render = self._merge_adjacent_text_blocks_in_same_page(cells_to_render)
                ...
                if md_content:
                    md_content = unescape_basic_sequences(md_content, convert_controls=False)
                    ...
                    if page_md_obj and page_md_obj.get("md_content"):
                        page_md_obj["md_content"] = unescape_basic_sequences(page_md_obj["md_content"], convert_controls=False)
                    return page_md_obj
    ...
    async def _run_inference_with_retries(
        ...
        *,
        ...
        while True:
            attempt += 1
            try:
                if use_race and self.concurrent_retries > 1:
                    result = await self._race_inference_attempts(payload, prompt, self.concurrent_retries)
                else:
    ...
    async def _process_single_page_optimized_streaming(self, page_data: dict):
        ...
        try:
            ...
            response, _ = await self._run_inference_with_retries(
                payload, prompt, original_page_num, use_race=False, max_attempts=primary_attempt_limit
            )
            last_raw_response_for_fallback = response

            cells, origin_image_for_saving = await _post_process_response(response)
            ...
            last_error = e
            console_write(
                f"⚠️ Page {original_page_num} primary attempt failed: {type(e).__name__}.",
                level="always"
            )
        ...
        if self.concurrent_retries and self.concurrent_retries > 1:
            try:
                ...
                console_write(
                    f"  Racing {self.concurrent_retries} concurrent requests for page {original_page_num}...",
                    level="always"
                )
                response, _ = await self._run_inference_with_retries(
                    payload, prompt, original_page_num, use_race=True, max_attempts=1
                )
                last_raw_response_for_fallback = response
    ...
    async def parse_image_simple(self, input_path, filename, prompt_mode, save_dir, bbox=None):
        ...
        try:
            ...
            if status in self.SUCCESS_STATUSES:
                ...
                final_md_text = unescape_basic_sequences(final_md_text, convert_controls=False)
