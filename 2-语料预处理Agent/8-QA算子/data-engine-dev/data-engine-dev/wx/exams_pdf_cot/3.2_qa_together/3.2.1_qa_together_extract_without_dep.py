...
def process_window(md_path: Path, s_line: int, e_line: int, chunk_text: str,
                   ...
                   out_root: Path, raw_root: Optional[Path], debug_dir: Optional[Path]) -> Tuple[str, int, int, List[QAItem], List[Dict[str, Any]]]:
    """
...
def process_one(md_path: Path,
                ...
                debug_dir: Optional[Path]) -> Tuple[str, List[QAItem], ExtractStats, List[Dict[str, Any]], Path, Path, Optional[Path]]:
    base_norm = md_path.stem
