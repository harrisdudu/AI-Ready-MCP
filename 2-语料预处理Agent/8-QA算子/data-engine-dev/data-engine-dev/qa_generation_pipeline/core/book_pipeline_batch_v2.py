...
from core.utils import extract_tags_and_content, extract_items, remove_image, remove_toc_lines_basic, trim_content
from langchain_text_splitters import RecursiveCharacterTextSplitter
from core.prompts import book_question_prompt, book_question_answer_prompt, evaluate_prompt_common, concept_extraction_prompt
...
def get_text_recall_local(model_path="/home/yehongxing/workspace/shi/QA_Generate/bge-m3", context_window_before: int = 1, context_window_after: int = 1):
    _text_recall = HierarchicalTextRecall(BGEEmbedding(model_path), context_window_before, context_window_after)
    return _text_recall
...
def _process_content_helper(file_path: str, content: str, content_splitter_params: dict, reference_splitter_params: dict) -> List[Dict[str, Any]]:
    ...
    for doc in documents:
        ...
        # 为每个reference记录其在当前content块中的位置索引
        references_docs = [reference_doc for reference_doc in references_docs if len(reference_doc.page_content) > 100]
        for idx, ref_doc in enumerate(references_docs):
            # if len(ref_doc.page_content) > 100:
            references.append({
                'text': ref_doc.page_content,
                'index': idx,  # 记录位置索引
                'total_in_chunk': len(references_docs),  # 记录总reference数量
                'content_hash': content_hash # 为content块生成唯一标识
            })
...
class BatchBookProcessor:
    def __init__(self, model_name: str, max_workers: int = 100, evaluate_model_name: str = None, context_window_before: int = 1, context_window_after: int = 1):
        ...
        self.recall_system = get_text_recall_local(Config.EMBEDDING_MODEL_PATH, context_window_before, context_window_after)
        self.content_splitter_params = {'chunk_size': 8000, 'chunk_overlap': 1000}
    ...
    async def process_step2(self, all_chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # 创建进度条
        pbar = tqdm(total=len(all_chunks) * self.max_try, desc="生成问题/脑图")
    ...
    async def _generate_questions_for_chunk(self, chunk: Dict[str, Any]) -> List[Dict[str, Any]]:
        """为内容块生成问题"""
        xmind_content = await self._generate_xmind(trim_content(chunk['content']), chunk['file_name'])
        prompt = book_question_prompt.format(
            content=xmind_content + "\n\n" + trim_content(chunk['content']),
        ...
        for question in questions:
            ...
            search_results = await self.recall_system.hierarchical_search(
                question, chunk['content'], chunk['file_path'], top_k=1
            )
    ...
    async def _generate_xmind(self, content: str, file_name: str) -> str:
        ...
        # 生成脑图提示词
        prompt = concept_extraction_prompt.format(text=content)
    ...
    async def _generate_answer_for_question(self, question_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 生成答案提示
        prompt = book_question_answer_prompt.format(
...
async def main():
    ...
    output_dir = os.path.join(output_dir, "book")
    os.makedirs(output_dir, exist_ok=True)
    max_workers = Config.MAX_WORKERS
    
    # 添加上下文窗口配置（可从Config获取或使用默认值）
    context_window_before = getattr(Config, 'CONTEXT_WINDOW_BEFORE', 1)
    context_window_after = getattr(Config, 'CONTEXT_WINDOW_AFTER', 1)
    
    # 获取所有Markdown文件
    file_paths = []
    for input_dir in input_dirs:
        file_paths += glob(os.path.join(input_dir, "**/*.md"), recursive=True)
    # file_paths = file_paths[10:20]
    if not file_paths:
    ...
    try:
        ...
        for i in range(0, len(file_paths), 100):
            ...
            try:
                ...
                logger.error(f"处理分片 {i//100 + 1} 时出错:")
                traceback.print_exc()
    finally:
        processor.shutdown()
